...
c$$$C          Note that PDG specific cut are more optimised than simple user cut
     ...
     &        .and. (abs(ipdg(i)).eq.12 .or. abs(ipdg(i)).eq.14
     &        .or.abs(ipdg(i)).eq.16)) then    ! top quark
     
C apply the pT cut (pT should be large than 200 GeV for the event to
C pass cuts)
            if (  pt_04(p(0,i)).le.40d0 ) then
C momenta do not pass cuts. Set passcuts_user to false and return
         ...
         if (istatus(i).eq.1    ! final state particle
     &        .and. (abs(ipdg(i)).eq.11 .or. abs(ipdg(i)).eq.13
     &        .or.abs(ipdg(i)).eq.15)) then    ! top quark
            do j=1,nexternal   ! loop over all external particles
                if (istatus(j).eq.1    ! final state particle
     &               .and. abs(ipdg(j)).eq.22) then
                  if (R2_04(p(0,i),p(0,j)).le.0.7d0**2) then
                    passcuts_user=.false.
                    return
                  endif
               endif
