...
#include "HiggsTemplateCrossSectionsStage1.h"
using HepMC::GenParticle;
using HepMC::GenVertex;

namespace Rivet {
  
  /// @class HiggsTemplateCrossSectionsStage1 
  ...
  class HiggsTemplateCrossSectionsStage1 : public Analysis {
  public:
    // Constructor
    HiggsTemplateCrossSectionsStage1()
      : Analysis("HiggsTemplateCrossSectionsStage1"),
  ...
  public:
    ...
    void init() {
      printf("==============================================================\n");
      printf("========     HiggsTemplateCrossSectionsStage1 Initialization     =========\n");
      printf("==============================================================\n");
    ...
    void analyze(const Event& event) {
      ...
      hist_stage0->fill( cat.stage0_cat/10*2+F);
      ...
      int off = offset[P];
      hist_stage1_pTjet25->fill(cat.stage1_cat_pTjet25GeV%100 + off);
      hist_stage1_pTjet30->fill(cat.stage1_cat_pTjet30GeV%100 + off);

      // Fill histograms: variables used in the categorization
      hist_pT_Higgs->fill(cat.higgs.pT());
      hist_y_Higgs->fill(cat.higgs.rapidity());
      hist_pT_V->fill(cat.V.pT());

      hist_Njets25->fill(cat.jets25.size());
      hist_Njets30->fill(cat.jets30.size());

      // Jet variables. Use jet collection with pT threshold at 30 GeV
      if (cat.jets30.size()) hist_pT_jet1->fill(cat.jets30[0].pt());
      if (cat.jets30.size()>=2) {
	const FourMomentum &j1 = cat.jets30[0].momentum(), &j2 = cat.jets30[1].momentum();
	hist_deltay_jj->fill(std::abs(j1.rapidity()-j2.rapidity()));
	hist_dijet_mass->fill((j1+j2).mass());
	hist_pT_Hjj->fill((j1+j2+cat.higgs.momentum()).pt());
      }
    }
  ...
  DECLARE_RIVET_PLUGIN(HiggsTemplateCrossSectionsStage1);
  //#endif
